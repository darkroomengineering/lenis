...
- ⏳ **Fullscreen wheel-driven slider** — viewport-wide slides on a horizontal rail: `orientation: 'horizontal'` + `gestureOrientation: 'both'` (a vertical wheel flick advances the rail), `infinite: true` (the rail loops), `drag` enabled, `lenis/snap` in `directional` + `lock` mode for one-slide-per-flick. References: [jesperlandberg.com](https://jesperlandberg.com/), [quentinhocde.com/works](https://quentinhocde.com/works/) — virtual-scroll rails (`overflow: hidden` + wheel/drag listeners), so they are missing snap and real horizontal scrolling (no native scroll position → no scrollbar, anchors, keyboard, reduced-motion)
