...
- ⏳ **Fullscreen wheel-driven slider** — viewport-wide slides on a horizontal rail: `orientation: 'horizontal'` + `gestureOrientation: 'both'` (a vertical wheel flick advances the rail), `infinite: true` (the rail loops), `drag` enabled, `lenis/snap` in `directional` + `lock` mode for one-slide-per-flick. Reference: [jesperlandberg.com](https://jesperlandberg.com/) — a virtual-scroll rail (`overflow: hidden` + wheel/drag listeners), so it is missing snap and real horizontal scrolling (no native scroll position → no scrollbar, anchors, keyboard, reduced-motion)
