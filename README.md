...
|-------------------------|----------------------------|----------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `allowNestedScroll`     | `boolean`                  | `false`                                            | Automatically smooth-scroll nested scrollable elements using child Lenis instances. Boundary detection ensures page scrolling resumes when a container hits its scroll limit. ⚠️ Can create performance issues since it checks the DOM tree on every scroll event. If that's a concern, use `prevent` option instead.          |
...
This automatically detects nested scrollable elements and smooth-scrolls them using child Lenis instances, giving them the same momentum and inertia as the page. When a container reaches its scroll limit, page scrolling resumes seamlessly. However, this can create performance issues since Lenis needs to check the DOM tree on every scroll event. If you experience performance problems, use `data-lenis-prevent` instead.
